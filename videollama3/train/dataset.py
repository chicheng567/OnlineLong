...
def get_frame_indices(
    ...
    if sample in ["rand", "middle"]:  # uniform sampling
        ...
        if max_num_frames > 0 and len(frame_indices) > max_num_frames:
            ids = np.linspace(
                0, len(frame_indices) - 1, num=max_num_frames, endpoint=True
            ).astype(np.int32)
            frame_indices = [
                frame_indices[i] for i in ids
...
def read_frames_gif(
    ...
    t_num_frames = num_frames  # np.random.randint(min_num_frames, num_frames + 1)
    frame_indices = get_frame_indices(
        t_num_frames,
        vlen,
        sample=sample,
...
def read_frames_decord(
    ...
    if clip:
        ...
        if start_index + vlen > len(video_reader):
            start_index = len(video_reader) - vlen
    ...
    if return_timestamps:
        timestamps = [round(f / fps, 1) for f in frame_indices]
...
class TCSLoader(object):
    ...
    def __call__(
        ...
        if image_type == "image":
            ...
            if fn.endswith("/") or os.path.isdir(fn):
                assert not return_timestamps
            ...
            #    frames = read_frames_av(fn, num_frames=max_num_frames, min_num_frames=min_num_frames, max_num_frames=max_num_frames,
...
def preprocess_videollama3(
    ...
    assert template_name == "videollama3", "Only videollama3 template is supported in this function."
...
def preprocess_internlm(
    ...
    conversations_origin = conversations
    ...
    for conversation, target in zip(conversations, targets):
        total_len = int(
            target.ne(tokenizer.pad_token_id).sum()
