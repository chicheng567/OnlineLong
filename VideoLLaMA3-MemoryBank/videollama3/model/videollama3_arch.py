...
class Videollama3MetaForCausalLM(ABC):
    ...
    def get_memory_bank(self):
        return self.get_model().get_memory_bank()
    ...
    def encode_images_with_memory(
        ...
        grid_sizes: torch.LongTensor,
        merge_sizes: torch.LongTensor,
        frame_indices: Optional[torch.LongTensor] = None,
        timestamps: Optional[torch.FloatTensor] = None,
        update_memory: bool = True,
    ) -> Tuple[torch.FloatTensor, Optional[dict]]:
        """
        Encode images with memory bank integration.
        
        Args:
            pixel_values: Input pixel values
            grid_sizes: Grid sizes for vision encoder
            merge_sizes: Merge sizes for vision encoder  
            frame_indices: Frame indices for memory tracking
            timestamps: Timestamps for temporal tracking
            update_memory: Whether to update memory bank with current features
        ...
        if self.has_memory_bank() and frame_indices is not None:
            ...
            if update_memory:
                ...
                for i in range(batch_size):
                    ...
                    # Reshape features back to spatial format for memory bank
                    frame_features = mm_features[i:i+1].unsqueeze(-1).unsqueeze(-1)  # Add spatial dims
            ...
            if mm_features.size(0) > 0:
                query_features = mm_features[0:1].unsqueeze(-1).unsqueeze(-1)
